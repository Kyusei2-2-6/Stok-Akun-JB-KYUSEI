...
(function () {
  ...
  bgm.volume = (!isNaN(savedVol)) ? Math.min(1, Math.max(0, savedVol)) : 0.32;
  ...
  bgm.addEventListener("error", function () {
    ...
    if (safeGet(STORAGE.warned404) !== "1") {
      ...
      console.warn("[music.js] Audio gagal dimuat. Cek path assets/music.mp3");
    }
    console.warn("[music.js] audio error", bgm.error, "src:", src);
  });

  // Save state
  bgm.addEventListener("play", function () { safeSet(STORAGE.playing, "1"); setDiscState(true); updateFab(true); hideEnableUI(); });
  bgm.addEventListener("pause", function () { safeSet(STORAGE.playing, "0"); setDiscState(false); updateFab(false); });
  ...
  function updateFab(isPlaying){
    var fab = document.getElementById("musicFab");
    if (!fab) return;
    var ico = fab.querySelector(".mfi");
    if (isPlaying) {
      fab.classList.add("isOn");
      if (ico) ico.textContent = "🔊";
      fab.setAttribute("aria-label","Music on");
      fab.title = "Music ON";
    } else {
      fab.classList.remove("isOn");
      if (ico) ico.textContent = "🔇";
      fab.setAttribute("aria-label","Music off");
      fab.title = "Music OFF";
    }
  }
  ...
  function rebind(){
    ...
    updateFab(!bgm.paused);
  }

  // --- Floating button (bottom-right) toggle
  (function bindFab(){
    var fab = document.getElementById("musicFab");
    if (!fab || fab.__bgmBound) return;
    fab.__bgmBound = true;
