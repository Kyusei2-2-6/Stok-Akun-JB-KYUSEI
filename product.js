/* =========================================================
   ...
   - Buy button routes to #/showcase?... so no reload => music stays on
   ========================================================= */
...
(function () {
  ...
  window.initProduct = function (params) {
    ...
    if (isSold) {
      ...
      // ---- 7) Build showcase route (hash) so no reload
    var showcaseUrl = null;
    if (!isSold) {
      showcaseUrl =
        "#/showcase?code=" + encodeURIComponent(product.code) +
