...
(function () {
  "use strict";
  // Fix common mojibake where UTF-8 bytes were accidentally stored as Latin-1 (e.g. "ð" instead of "🌐")
  function fixMojibake(str) {
    if (typeof str !== "string") return String(str || "");
    // Heuristic: only try when it looks like mojibake
    if (!/[ÃÂâð]/.test(str)) return str;

    // 1) Preferred: TextDecoder (modern browsers)
    try {
      if (typeof TextDecoder !== "undefined") {
        var bytes = new Uint8Array(str.length);
        for (var i = 0; i < str.length; i++) bytes[i] = str.charCodeAt(i) & 0xff;
        var decoded = new TextDecoder("utf-8").decode(bytes);
        if (decoded.indexOf("�") === -1) return decoded; // avoid breaking normal text
      }
    } catch (_) {}

    // 2) Fallback: decodeURIComponent(escape(...)) (legacy)
    try {
      var decoded2 = decodeURIComponent(escape(str));
      if (decoded2 && decoded2.indexOf("�") === -1) return decoded2;
    } catch (_) {}
  ...
  function getParam(params, key) {
    ...
    // router.js passes a plain object: { code: "A001" }
    var v =
      params[key] ??
      params[String(key).toLowerCase()] ??
      params[String(key).toUpperCase()];
  ...
  window.initProduct = function (params) {
    ...
    function openDetail() {
      ...
      if (!Array.isArray(details) || details.length === 0) {
        ...
        for (var j = 0; j < details.length; j++) {
          ...
          li.textContent = fixMojibake(String(details[j]));
    ...
    // ---- QRIS buy flow (static, no backend)
    (function bindBuyFlow(){
      var buyBtn = document.getElementById("buyProductBtn");
      var qrModal = document.getElementById("qrModal");
      var qrClose = document.getElementById("qrClose");
      var qrProduct = document.getElementById("qrProduct");
      var qrPrice = document.getElementById("qrPrice");
      var qrWA = document.getElementById("qrWA");
      var qrTG = document.getElementById("qrTG");
      var qrFB = document.getElementById("qrFB");
      var waUrl = "https://wa.me/6283863831670";
      var tgUrl = "https://t.me/DedySetyadi226";
      var fbUrl = "https://www.facebook.com/kyu.sei.924076";
      function message(){
        return "Halo admin, saya mau beli akun " + (product.code || "-") + " - " + (product.name || "Produk") + " harga " + rupiah(product.price) + ". Saya sudah bayar, ini bukti pembayarannya.";
      }
      function openQR(){
        if (!qrModal) return;
        if (qrProduct) qrProduct.textContent = (product.code || "-") + " • " + (product.name || "Produk");
        if (qrPrice) qrPrice.textContent = rupiah(product.price);
      ...
      if (buyBtn) {
        if (isSold) {
          buyBtn.textContent = "Produk Sold";
          buyBtn.disabled = true;
          buyBtn.style.opacity = ".55";
        } else {
          buyBtn.onclick = openQR;
        }
      }
      if (qrClose) qrClose.onclick = closeQR;
      if (qrModal) qrModal.addEventListener("click", function(e){ if (e.target === qrModal) closeQR(); });
      if (qrWA) qrWA.onclick = function(){ window.open(waUrl + "?text=" + encodeURIComponent(message()), "_blank"); };
      if (qrTG) qrTG.onclick = function(){ window.open(tgUrl, "_blank"); };
      if (qrFB) qrFB.onclick = function(){ window.open(fbUrl, "_blank"); };
    })();
