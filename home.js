...
(function () {
  ...
  function getGames(){
    // Prefer explicit GAMES from data.js (editable by bot)
    if (typeof GAMES !== "undefined" && Array.isArray(GAMES) && GAMES.length){
      return GAMES.map(function(g){
        return {
          key: String(g.key || g.game || "").toLowerCase(),
          name: String(g.name || g.label || g.key || "").trim()
        };
      }).filter(function(g){ return g.key && g.name; });
    }
    // Fallback: derive from PRODUCTS
    var set = {};
    safeArr(window.PRODUCTS).forEach(function(p){
      var k = String(p.game || "").toLowerCase();
      if (!k) return;
      if (!set[k]) set[k] = k.toUpperCase();
    });
    return Object.keys(set).map(function(k){ return { key:k, name:set[k] }; });
  }
  ...
  function renderGameDropdown(){
    ...
    var games = getGames();
    var html = [];
    html.push('<button class="ddItem" data-game="all" type="button">Semua</button>');
    games.forEach(function(g){
      html.push('<button class="ddItem" data-game="'+escapeHtml(g.key)+'" type="button">'+escapeHtml(g.name)+'</button>');
    });
