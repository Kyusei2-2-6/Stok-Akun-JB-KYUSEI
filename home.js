/* =========================================================
   ...
   - Validasi URL harus hash route (#/product...)
...
(function () {
  ...
  function isValidResumeUrl(url, kind) {
    ...
    // pay route dihapus (showcase only)
  ...
  function initWelcome() {
    ...
    var welcomeMusicBtn = $("welcomeMusicBtn");
    ...
    // IMPORTANT:
    // Browser (Chrome/Android/iOS) will BLOCK audio play() unless it is called
    // directly inside a user gesture handler (click/tap), not inside setTimeout.
    // So for "Masuk + 🔊" we call play() immediately in the click handler.
    function startMusicNow() {
      if (!window.__kyuseiMusic) return;
      try {
        try { localStorage.setItem("bgm_playing", "1"); } catch (e) {}
        window.__kyuseiMusic.play().catch(function () { /* blocked -> user can tap music button */ });
      } catch (e) {}
    }
    ...
    if (welcomeMusicBtn) {
      welcomeMusicBtn.addEventListener("click", function(){
        // MUST start music inside this click handler (anti autoplay block)
        startMusicNow();
        closeWelcomeAnimated();
      });
    }
  ...
  window.initHome = function () {
    ...
    // Pembayaran dihapus (showcase only)

    var hasProduct = setResumeLink(resumeProduct, "lastProductUrl", "product");

    if (resumeWrap) resumeWrap.style.display = (hasProduct) ? "flex" : "none";
    ...
    function applyFilter(game) {
      ...
      var list = [];
      var popular = { genshin:1, ml:1, toram:1, hsr:1, ff:1, pubgm:1, valorant:1, roblox:1, codm:1 };
      if (currentGame === "all") list = PRODUCTS.slice();
      else if (currentGame === "popular") {
        for (var i = 0; i < PRODUCTS.length; i++) {
          var g = (PRODUCTS[i].game || "").toLowerCase();
          if (popular[g]) list.push(PRODUCTS[i]);
        }
      } else {
        for (var i = 0; i < PRODUCTS.length; i++) {
          var g = (PRODUCTS[i].game || "").toLowerCase();
